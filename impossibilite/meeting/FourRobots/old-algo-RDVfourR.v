...
(** Small dedicated decision tactic for reals handling 1<>0 and -1<>0 *)
Ltac Rdec := repeat
  match goal with
    | |- context[Rdec ?x ?x] =>
...
Lemma forbidden_invariant_aux : forall p q (σ : permutation (ident Four Zero)), p ∘ σ ≈ q -> forbidden q -> forbidden p.
Proof.
...
Lemma forbidden_invariant : forall p q (σ : permutation (ident Four Zero)), p ∘ σ ≈ q -> (forbidden q <-> forbidden p).
Proof.
...
(** Symmetric position **)
Definition symmetric (pos : position Four Zero) :=
  exists σ : permutation (ident Four Zero),
  (pos ∘ σ).(gp) One4 - center4 pos = - ((pos ∘ σ).(gp) Four4 - center4 pos)
  /\ (pos ∘ σ).(gp) Two4 - center4 pos = - ((pos ∘ σ).(gp) Three4 - center4 pos).
...
Definition is_symmetric c (pos : position Four Zero) : bool :=
  let '(x, y, z, t) :=
    sort4 Rle_lt_dec (pos.(gp) One4) (pos.(gp) Two4) (pos.(gp) Three4) (pos.(gp) Four4) in
  if Rdec (x - c) (-(t - c)) then if Rdec (y - c) (-(z - c)) then true else false else false.

Theorem is_symmetric_correct : forall pos, is_symmetric (center4 pos) pos = true -> symmetric pos.
Proof.
intro pos. unfold is_symmetric; simpl.
pose (c := center4 pos). fold c.
...
destruct (sort4 Rle_lt_dec (gp pos One4) (gp pos Two4) (gp pos Three4) (gp pos Four4))
  as [[[x y] z] t].
destruct H4sort as [[H1 H2] H3].
destruct H4perm as [σ Heq].
intro H. exists σ. simpl. inversion Heq. rewrite H4,H5,H6,H7. fold c.
revert H. Rdec_full; try Rdec_full; easy || now rewrite Heq0, Heq1.
(* Converse implication?
+ inversion Heq. rewrite H4,H5,H6,H7.
  destruct H as [σ' [Hsym1 Hsym2]].
  repeat Rdec_full; reflexivity || elim Hneq. *)
Qed.

Lemma is_symmetric_invariant : forall c p q σ, p ∘ σ ≈ q -> is_symmetric c p = is_symmetric c q.
Proof. intros c p q σ Hpq. unfold is_symmetric. rewrite <- (sort4_invariant Hpq). reflexivity. Qed.

Definition closest center (pos : position Four Zero) :=
  let '(x, y, z, t) := sort4 Rle_lt_dec (pos.(gp) One4) (pos.(gp) Two4) (pos.(gp) Three4) (pos.(gp) Four4) in
  Rmin (Rabs (y - center)) (Rabs (z - center)).

Lemma closest_invariant : forall c (p q : position Four Zero) σ, p ∘ σ ≈ q -> closest c p = closest c q.
Proof. intros c p q σ Hpq. unfold closest. rewrite <- (sort4_invariant Hpq). reflexivity. Qed.
...
+ split; intro H. discriminate H. exfalso. clear H1.
  ...
  assert (Habs : p1 = t).
  { rewrite <- Hp1. simpl. rewrite <- Hn. change (locate pos ((σ' ∘∘ (σ' ⁻¹)) (σ (Good Four _ Four4))) = t).
    rewrite compose_inverse_r. simpl. now inversion Heq. }
  rewrite Habs in *. case_eq (σ⁻¹ (σ' (Good Four _ Two4))); intros []; intro Hm.
    rewrite Inversion in Hm, Hn. simpl in Hm, Hn.

  inversion Heq. rewrite H0 in H1. destruct (σ (Good Four Zero Four4)) as [[] | []].
Admitted.
...
Definition robogram4 (pos : position Four Zero) : location :=
  let c := center4 pos in
  if is_forbidden pos then 0 else
  match has_stack pos with
    | Some p => p
    | None => if is_symmetric c pos then c else closest c pos end.

Lemma robogram4_morph : forall (p q : position Four Zero) (σ : permutation _), q ≈ p ∘ σ -> robogram4 p = robogram4 q.
Proof.
...
+ rewrite is_forbidden_false in Hf. symmetry in Hσ. assert (Hff := Hf).
  ...
  rewrite (center4_invariant σ Hσ).
  rewrite (has_stack_invariant σ Hf Hσ).
  rewrite (is_symmetric_invariant (center4 q) Hσ).
  rewrite (closest_invariant (center4 q) Hσ).
  reflexivity.
Qed.
