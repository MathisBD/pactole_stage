...
Proof.
intros A B C f g l. induction l; intro x; simpl.
  reflexivity.
  now rewrite IHl.
Qed.

Lemma fold_left_cons_length_aux : forall A B (f : A -> B) l1 l2,
  length (fold_left (fun acc x => f x :: acc) l1 l2) = length l1 + length l2.
Proof.
intros A B f l1. induction l1; intro l2; simpl.
  reflexivity.
  rewrite IHl1. simpl. omega.
Qed.

Corollary fold_left_cons_length : forall A B (f : A -> B) l,
  length (fold_left (fun acc x => f x :: acc) l nil) = length l.
Proof. intros. rewrite fold_left_cons_length_aux. simpl. omega. Qed.

Lemma InA_map_iff: forall (A B : Type) (f : A -> B) (l : list A) (y : B) eqA eqB,
  Equivalence eqA -> Equivalence eqB -> Proper (eqA ==> eqB) f ->
  (InA eqB y (map f l) <-> (exists x : A, eqB (f x) y /\ InA eqA x l)).
Proof.
intros A B f l y eqA eqB HeqA HeqB Hf. induction l.
+ split; intro H.
    exfalso. rewrite <- InA_nil. now apply H.
    destruct H as [? [_ Habs]]. now inversion Habs.
+ simpl. split; intro H.
    inversion_clear H.
      exists a. split. now symmetry. now left.
      rewrite IHl in H0. destruct H0 as [x [Hy Hin]]. exists x. split. assumption. now right.
    destruct H as [x [Hy Hin]]. inversion_clear Hin.
      left. rewrite (Hf x a H) in Hy. now symmetry.
      right. rewrite IHl. exists x. now split.
Qed.

Lemma NoDupA_inj_map :
  forall (A B : Type) (f : A -> B) (l : list A) eqA eqB,
  Equivalence eqA -> Equivalence eqB -> Proper (eqA ==> eqB) f ->
  (forall x y, eqB (f x) (f y) -> eqA x y) -> (NoDupA eqB (map f l) <-> NoDupA eqA l).
Proof.
intros A B f l eqA eqB HeqA HeqB Hf Hinj. induction l; simpl.
  split; constructor.
...
Lemma NoDupA_app_iff : forall (A : Type) (eqA : A -> A -> Prop), Equivalence eqA ->
  forall l l' : list A, NoDupA eqA (l ++ l')
  <-> NoDupA eqA l /\ NoDupA eqA l' /\ (forall x : A, InA eqA x l -> InA eqA x l' -> False).
Proof.
intros A eqA HeqA l l'. split; intro H; try now apply NoDupA_app.
...
Lemma NoDupA_swap_iff : forall (A : Type) (eqA : A -> A -> Prop), Equivalence eqA ->
  forall (l l' : list A) (x : A), NoDupA eqA (l ++ x :: l') <-> NoDupA eqA (x :: l ++ l').
Proof.
intros A eqA HeqA l l'. split; intro H. now apply NoDupA_swap.
...
Definition remove (x : elt) (n : nat) (s : t) : t :=
  ...
  if le_lt_dec (multiplicity x s) n then M.remove x s
                                    else M.add x (Pos.of_nat (multiplicity x s - n)) s.
(*
Definition add x n s := M.add x (multiplicity x s + n) s.
Definition singleton x n := M.add x n (M.empty nat).
Definition remove x n s := M.add x (multiplicity x s - n) s.
Definition union (s s' : t) := M.fold add (projS1 s) s'.
Definition diff (s s' : t) := M.fold remove (projS1 s) s'.
*)
...
Definition Empty s := forall x, multiplicity x s = 0.
...
+ apply M.is_empty_2 in H. intros x. unfold M.Empty in H. unfold multiplicity. destruct (M.find x s) eqn:Hin.
  - elim (H x p). now apply M.find_2.
  - reflexivity.
+ apply M.is_empty_1. intros x n Habs. apply M.find_1 in Habs. specialize (H x).
...
intros x s Hs. unfold multiplicity. destruct (M.find x s) eqn:Hin.
  rewrite is_empty_spec in Hs. elim (Hs x). unfold In, multiplicity. now rewrite Hin.
  reflexivity.
Qed.

Corollary empty_multiplicity : forall x, multiplicity x empty = 0.
Proof. intro. apply is_empty_multiplicity. rewrite is_empty_spec. apply empty_spec. Qed.

Lemma add_spec : forall x n s, multiplicity x (add x n s) = multiplicity x s + n.
Proof.
...
Lemma add_spec' : forall x y n s, ~E.eq x y -> multiplicity y (add x n s) = multiplicity y s.
intros x y n s Hxy. unfold add. destruct (eq_nat_dec n 0). reflexivity. unfold multiplicity at 1 3. simpl.
destruct (M.find y s) eqn:Heq.
...
Lemma remove_spec : forall x n s, multiplicity x (remove x n s) = multiplicity x s - n.
Proof.
...
Lemma remove_spec' : forall x y n s, ~E.eq x y -> multiplicity y (remove x n s) = multiplicity y s.
Proof.
intros x y n s Hxy. unfold remove. destruct (eq_nat_dec n 0). reflexivity.
destruct (le_lt_dec (multiplicity x s) n) as [Hle | Hlt]; unfold multiplicity at 1; simpl.
...
Proof.
intros x y n. unfold singleton, add. destruct (eq_nat_dec n 0).
  subst n. now destruct (E.eq_dec y x); apply empty_spec.
...
intros f x n l s Hin Hdup. revert s. induction l as [| [y m] l]; intro s; simpl.
  reflexivity.
  rewrite IHl. apply add_spec'. intro Habs. apply Hin. left. compute. auto. auto. now inversion_clear Hdup.
Qed.
...
revert s'. induction (M.elements s) as [| [y m] l]; intro s'; simpl.
  ...
  rewrite IHl. apply add_spec'. now auto.
...
revert s'. induction (M.elements s) as [| [y m] l]; intro s'; simpl.
  ...
  + transitivity (multiplicity x (add y (f y (Pos.to_nat n)) s')).
    - rewrite (fold_add_out_list (fun x y => f x (Pos.to_nat y)) x n).
        reflexivity.
        inversion_clear Hdup. clear -H H1. intro Habs. apply H. revert Habs. apply InA_eqA.
          now apply Meq_key_equiv.
          assumption.
        now inversion_clear Hdup.
    - rewrite H1, add_spec, plus_comm. f_equal. unfold multiplicity.
      apply (M.MapsTo_1 H1) in Hin. apply M.find_1 in Hin. now rewrite Hin.
  + rewrite IHl.
      f_equal. apply add_spec'. intro Habs. apply Hneq. compute. split. now auto.
      apply (NoDup_pair_In _ x y n m ((y, m) :: l)); trivial. left. compute. now auto. now auto.
...
+ rewrite (fold_add (fun x n => min (multiplicity x s) n) x s' empty).
    rewrite (empty_multiplicity x). now rewrite plus_0_r.
    intros ? ? Heq ? ? ?. simpl. subst. now rewrite Heq.
     exists p. now apply M.find_2.
+ rewrite fold_add_out.
    unfold multiplicity at 3. rewrite Hin. rewrite Min.min_0_r. now apply empty_multiplicity.
    intros [n Habs]. apply M.find_1 in Habs. rewrite Hin in Habs. discriminate Habs.
Qed.
...
+ rewrite (fold_add (fun x n => n - multiplicity x s') x s empty).
    rewrite empty_multiplicity. now rewrite plus_0_r.
    intros ? ? Heq ? ? ?. subst. now rewrite Heq.
     exists p. now apply M.find_2.
+ rewrite fold_add_out.
    unfold multiplicity at 2. rewrite Hin. simpl. now apply empty_multiplicity.
    intros [n Habs]. apply M.find_1 in Habs. rewrite Hin in Habs. discriminate Habs.
Qed.
...
Lemma Melements_multiplicity : forall s xn,
  InA (@M.eq_key_elt _) xn (M.elements s)
  <-> multiplicity (fst xn) s = Pos.to_nat (snd xn) /\ Pos.to_nat (snd xn) > 0.
Proof.
intros s [x n]. simpl.
split; intro H.
...
split; intro H.
  ...
  { rewrite InA_map_iff in Hn. destruct Hn as [[y m] [Heq Hin]]. rewrite Melements_multiplicity in Hin.
    destruct Hin as [Hin _]. destruct Heq as [Heq1 Heq2]. hnf in *. simpl in *. rewrite <- Heq1. now subst n.
    now apply Meq_equiv. now apply eq_pair_equiv. intros ? ? H. now rewrite H. }
  now subst n.
...
intros f x n l s Hin Hdup. revert s. induction l as [| [y m] l]; intro s; simpl.
  reflexivity.
  rewrite IHl.
    destruct (f y m). apply add_spec'. intro Habs. apply Hin. left. compute. auto. auto.
...
* destruct a as [y m].
  ...
  + destruct Heq as [H1 H2]. simpl in H1, H2 |- *. rewrite H1 in *. subst m.
    ...
    - rewrite Nat2Pos.id; try omega.
      destruct (f y (multiplicity y s)) eqn:Htest.
        rewrite add_spec, H1. now apply plus_comm.
        now rewrite H1.
  ...
  + inversion_clear Hs.
    ...
    - inversion_clear Hdup. assert (Hxy : ~E.eq x y).
      ...
      reflexivity || rewrite add_spec'; reflexivity || now auto.
Qed.
...
revert s'. induction (M.elements s) as [| [y m] l]; intro s'; simpl.
  reflexivity.
  assert ( Hxy : ~E.eq x y). { intro Habs. apply (Hs m). left. now split. }
  rewrite IHl. destruct (f y (Pos.to_nat m)). apply add_spec'. now auto. reflexivity.
  intros n Habs. apply (Hs n). now right.
Qed.
...
destruct (multiplicity x s) eqn:Hin.
  rewrite filter_spec_out.
    now destruct (f x 0); apply empty_multiplicity.
    unfold In. omega.
  rewrite filter_spec_In.
    now rewrite empty_multiplicity, Hin, plus_0_r.
    assumption.
...
Lemma choose_spec1 : forall x s, choose s = Some x -> In x s.
Proof.
...
Proof.
intros s Hs x. unfold choose in Hs. rewrite M.fold_1 in Hs. rewrite multiplicity_out.
...
+ destruct H as [Hb H]. subst b.
  ...
  { intros x Hx. rewrite Melements_multiplicity in Hx. destruct Hx as [Hm Hp].
    setoid_rewrite <- Pos2Nat.id. rewrite <- Hm. rewrite Nat2Pos.id. apply H. unfold In. now rewrite Hm. omega. }
  clear H. revert H0. induction (M.elements s); simpl; intro Hin.
    reflexivity.
...
Proof.
intros f s Hf. unfold for_all. rewrite for_all_spec_aux.
  now intuition.
...
Proof.
intros f s Hf. unfold exists_. rewrite exists_spec_aux.
  intuition. now inversion H0.
  intros ? ? H1 ? ? H2. now rewrite H1, H2.
Qed.
...
intros f x n l s12 Hin Hdup. revert s12. induction l as [| [y m] l]; intros s12; simpl.
  ...
  rewrite IHl.
    destruct (f y (Pos.to_nat m)); simpl.
      apply add_spec'. intro Habs. apply Hin. left. compute. auto. auto.
    intro Habs. apply Hin. now right.
    now inversion_clear Hdup.
Qed.
...
* destruct a as [y m].
  ...
  + inversion_clear Hs.
    ...
    - inversion_clear Hdup. assert (Hxy : ~E.eq x y).
      ...
      destruct (f x (multiplicity x s)), (f y (Pos.to_nat m));
      reflexivity || simpl; rewrite add_spec'; reflexivity || now auto.
Qed.
...
revert s'. induction (M.elements s) as [| [y m] l]; intro s'; simpl.
  reflexivity.
  assert ( Hxy : ~E.eq x y). { intro Habs. apply (Hs m). left. now split. }
  rewrite IHl. unfold partition_fun. destruct (f y (Pos.to_nat m)). apply add_spec'. now auto. reflexivity.
  intros n Habs. apply (Hs n). now right.
Qed.
...
intros f x n l s12 Hin Hdup. revert s12. induction l as [| [y m] l]; intros s12; simpl.
  reflexivity.
  rewrite IHl.
    destruct (f y (Pos.to_nat m)). reflexivity. simpl. apply add_spec'. intro Habs. apply Hin. left. compute. auto.
    intro Habs. apply Hin. now right.
    now inversion_clear Hdup.
Qed.
...
revert s'. induction (M.elements s) as [| [y m] l]; intro s'; simpl.
  reflexivity.
  assert ( Hxy : ~E.eq x y). { intro Habs. apply (Hs m). left. now split. }
  rewrite IHl. destruct (f y (Pos.to_nat m)); simpl. reflexivity. apply add_spec'. now auto.
  intros n Habs. apply (Hs n). now right.
Qed.
...
destruct (multiplicity x s) eqn:Hin.
  ...
  rewrite partition_spec_snd_In.
    rewrite empty_multiplicity, Hin, plus_0_r. now destruct (f x (S n)).
    assumption.
    unfold In. rewrite Hin. omega.
    intros ? ? Heq ? ? ?. subst. now rewrite Heq.
Qed.

Instance fold_compat A (eqA : A -> A -> Prop) `{Equivalence A eqA} :
...
rewrite fold_left_symmetry_PermutationA.
  ...
  apply NoDupA_equivlistA_PermutationA.
    ...
    eapply NoDupA_strengthen; try apply M.elements_3w. now intros [? ?] [? ?] [? ?].
    clear -Hs. intro x. do 2 rewrite Melements_multiplicity. now rewrite Hs.
  assumption.
Qed.
