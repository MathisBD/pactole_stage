...
Definition PosEq (pos₁ pos₂ : position) : Prop := ∀id, locate pos₁ id = locate pos₂ id.
...
Inductive phase :=
  | Off
  | Observe (obs : location -> automorphism location).
...
Definition round (r : robogram) (da : demonic_action) (gp : G → location) : G → location :=
  ...
  fun g => 
    ...
    match da.(step) g with (** first see whether the robot is activated *)
      | Off => t (** If g is not activated, do nothing *)
      | Observe f => (** the zooming factor set by the demon *)
        let spectr := da.(spectrum_of) g in
          (** otherwise compute the position [g] actually sees, apply robogram
            [r] on it and translate the destination location back in the global
            reference *)
         let pos_seen_by_r := f {| gp := gp; bp := locate_byz da |} in
          f ⁻¹ (r (spectr pos_seen_by_r))
    end.
