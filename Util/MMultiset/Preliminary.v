...
Proof using . intros n m. do 2 rewrite Nat.compare_eq_iff. now split. Qed.

Lemma nat_compare_Lt_Gt : forall n m, Nat.compare n m = Lt <-> Nat.compare m n = Gt.
Proof using . intros n m. rewrite <- nat_compare_lt, <- nat_compare_gt. now split. Qed.
...
Proof using . intros f g Hf Hg x y Heq. now apply Hg, Hf. Qed.
...
Proof using . intros [| ? [| ? ?]]; simpl; split; try discriminate; eauto; intros [? ?]; discriminate. Qed.
...
Proof using . intros l l' Hl. induction Hl; constructor; assumption. Qed.
...
Proof using HeqA. apply (removeA_InA_iff_strong eq_dec). reflexivity. Qed.
...
Proof using HeqA. intros. intro. rewrite removeA_InA_iff_strong; tauto. Qed.
...
Proof using . intros x y ? ? ?. rewrite removeA_app. simpl. destruct (eq_dec x y). contradiction. reflexivity. Qed.
...
Proof using HeqA. intros ? ? Hperm1 ? ? Hperm2. unfold inclA. setoid_rewrite Hperm1. setoid_rewrite Hperm2. reflexivity. Qed.
...
Proof using . intros. apply plus_minus. rewrite <- (partition_length f), partition_filter. simpl. apply plus_comm. Qed.
