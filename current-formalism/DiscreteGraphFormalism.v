...
+ specialize (H (Good g)); destruct H as (l, (Hl, (Hs, Ht))).
  ...
  - destruct (Rdec dist 0); intros v0 Hl0'.
    ...
    destruct (Rdec dist 1); simpl in *. now right.
    rewrite Ht' in *.
    assert (Hleq : loc_eq (Loc l3) (Loc l2)).
    rewrite Hl'. simpl in *. now rewrite Hl, Ht.
    simpl in *.
