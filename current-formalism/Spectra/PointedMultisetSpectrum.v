...
Section PointedMultisetSpectrum.
...
Lemma pspect_from_config_map  : forall f, Proper (equiv ==> equiv) f ->
  ...
  map f (snd (pspect_from_config config pt)) == snd (pspect_from_config (map_config f config) pt).
Proof.
repeat intro. unfold pspect_from_config, multiset_pspectrum. cbn [snd].
now rewrite config_list_map, map_map, <- make_multiset_map, map_map.
Qed.

Theorem cardinal_pspect_from_config : forall config pt, cardinal (snd (pspect_from_config config pt)) = nG + nB.
Proof.
intros. unfold pspect_from_config, multiset_pspectrum. cbn [snd].
now rewrite cardinal_make_multiset, map_length, config_list_length.
Qed.

Property pos_in_config : forall config pt id, In (fst (config id)) (snd (pspect_from_config config pt)).
Proof.
intros config pt id. cbn. unfold In.
rewrite make_multiset_spec, (countA_occ_pos _), InA_map_iff.
...
Property spect_from_config_In : forall config pt x,
  In x (snd (pspect_from_config config pt)) <-> exists id, fst (config id) == x.
Proof.
intros config pt x. split; intro Hin.
+ assert (Heq := pspect_from_config_spec config pt).
  unfold pspect_is_ok, pspect_from_config, multiset_pspectrum in *.
...
End PointedMultisetSpectrum.

Global Notation "s [ x ]" := (multiplicity x (snd s)) (at level 2, no associativity, format "s [ x ]").
