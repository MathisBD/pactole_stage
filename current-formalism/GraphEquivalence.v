...
Module AGF := AGF (Graph)(N)(Names)(LocationA)(ConfigA)(Iso)(Mraw)(M).
Module DGF := DGF (Graph)(N)(Names)(LocationA)(ConfigA)(Iso).
...
Lemma Mraw_equiv : AGF.Spect.t = DGF.Spect.t.
  unfold AGF.Spect.t, DGF.Spect.t.
  unfold AGF.Spect.Mraw.M.t.
    
  

  Lemma spect_equiv : forall conf, 

Definition SpectA2D (sA : AGF.Spect.t)  : DGF.Spect.t.
...
Definition rbgA2D (rbgA : AGF.robogram) : DGF.robogram.
  refine {| DGF.pgm := fun s g => DGF.Loc ((AGF.pgm rbgA) s g) |}.
Proof.
+ intros SD1 SD2 HSD g1 g2 Hg. unfold DGF.Location.eq, DGF.loc_eq, DGF.Spect.eq in *.
  apply (AGF.pgm_compat rbgA). now unfold AGF.Spect.eq. assumption.
+ intros s g l Hl.  
  assert (Hpgm := AGF.pgm_range rbgA s l g Hl).
...
refine {| AGF.pgm := fun s g => LocD2A ((DGF.pgm rbgD) s g) |}.
Proof.
+ intros SA1 SA2 HSA g1 g2 Hg. unfold AGF.Location.eq, AGF.Spect.eq in *.
...
Proof.
intros rd1 rd2 Hrd sa1 sa2 Hsa g1 g2 Hg. unfold rbgD2A, AGF.req. simpl.
...
destruct id as [g|b]; try (now exfalso); simpl in *; rewrite HstepA in *; try discriminate.
 ...
 - simpl in *. rewrite HtgtA.
   unfold DGF.Spect.from_config, AGF.Spect.from_config.
   unfold DGF.projectS, DGF.projectS_loc.
   simpl.
   unfold AGF.Config.map, AGF.apply_sim.
   assert (HIso_eq : DGF.Aom_eq (DGF.Active sim) (DGF.Active sim0)) by now rewrite HstepD.
   unfold DGF.Aom_eq in HIso_eq.
   f_equiv.
   apply HIso_eq.
   f_equiv.
   unfold DGF.Spect.eq, DGF.View.eq.
...
(forall g, DGF.group_good_def c g) ->
...
try (now (try (now simpl in *);
  ...
  destruct n; unfold rcD2A, LocD2A; rewrite HlocD, HtgtD, HsrcD; repeat try split; try now simpl)).
     + unfold ConfigD2A, LocD2A in *; simpl in *;
         destruct (DGF.Config.loc (c (Good g))) eqn : Habsurd; rewrite HlocD in Habsurd; try discriminate.
  ...
  - destruct (DGF.Config.loc (c' (Good g))) as [lld' | eld' pld' ] eqn : HlocD';
    ...
    * destruct (Rdec dist0 0) eqn : Hdist.
      ...
      ** destruct (Rle_dec (dist0) (Graph.threshold e1)); try discriminate; simpl in *.
         ...
         destruct (AGF.Config.eq_RobotConf_dec
               ...
               AGF.Config.loc := l;
               AGF.Config.robot_info := {|
                                        AGF.Config.source := lsd;
                                        AGF.Config.target := ltd |} |}
               (rcD2A (c (Good g)))); try contradiction; try discriminate.
      ...
      ** destruct (AGF.Config.eq_RobotConf_dec
               ...
               AGF.Config.robot_info := {|
                                        AGF.Config.source := lsd;
                                        AGF.Config.target := ltd |} |}); discriminate.
    ...
    * rewrite HsrcD, HtgtD in *.
      ...
      destruct ( Rdec dist0 0); simpl in *.
      rewrite Habsurd in *.
      assert (HlocD_aux: DGF.Location.eq (DGF.Loc l) (DGF.Loc lld))
    ...
    * destruct (Rle_dec (DGF.project_p pld') (Graph.threshold eld'));
      ...
      assert (HlocD_aux: DGF.Location.eq (DGF.Loc l) (DGF.Loc lld))
        by now rewrite HlocD.
      unfold DGF.Location.eq, DGF.loc_eq in *.
      now destruct (DGF.Config.loc (c (Good g))).
      destruct (Rdec dist0 1); simpl in *.
      now exfalso.
  ...
  - simpl in *. rewrite HsrcD, HtgtD in *.
    ...
    assert (DGF.Location.eq (DGF.Loc l) (DGF.Loc lld)) by (now rewrite HlocD);
    unfold DGF.Location.eq,  DGF.loc_eq in *;
    now symmetry));
    try (now (assert (DGF.Location.eq (DGF.Loc l) (DGF.Loc lld)) by now rewrite HlocD;
    now unfold DGF.Location.eq, DGF.loc_eq in *)). 
    destruct Hloc as (Heeq, Hpeq).
    ...
    try (assert (DGF.Location.eq (DGF.Loc l) (DGF.Loc lld)) by (now rewrite HlocD);
    unfold DGF.Location.eq, DGF.loc_eq in *). 
    now rewrite H in *.
    symmetry in Hpeq.
    rewrite <- (DGF.subj_proj dist0 pld') in Hpeq.
...
+ unfold ConfigD2A, LocD2A in *; rewrite HlocD, HtgtD, HsrcD in *.
...
+ destruct dist1 eqn : Hbool;
  ...
  try (destruct (Rle_dec 1 (DGF.project_p pld + dist0)); simpl in *;
      rewrite HlocD, HsrcD, HtgtD in *; now simpl in *); try now simpl in *.
+ unfold ConfigD2A, LocD2A in *; rewrite HlocD in *.
  destruct dist1 eqn : Hbool;
  destruct (DGF.Config.loc (c' (Byz b))) as [lld' | eld' pld'] eqn : HlocD';
  destruct (DGF.Config.source (DGF.Config.robot_info (c (Byz b)))) as [lsd | esd psd] eqn : HsrcD;
  destruct (DGF.Config.source (DGF.Config.robot_info (c' (Byz b)))) as [lsd' | esd' psd'] eqn : HsrcD';
...
+ unfold ConfigD2A, LocD2A in *; rewrite HlocD, HtgtD, HsrcD in *.
  ...
  assumption.
+ destruct dist;
  destruct (AGF.Config.eq_RobotConf_dec (ConfigD2A c (Good g)) (rcD2A (c (Good g))));
    try discriminate.
  destruct n.
  unfold rcD2A, ConfigD2A, LocD2A in *.
  rewrite HlocD, HtgtD, HsrcD.
  reflexivity.
+ destruct dist,
  (AGF.Config.eq_RobotConf_dec (ConfigD2A c (Good g)) (rcD2A (c (Good g))));
    try discriminate.
  destruct n.
  unfold ConfigD2A, rcD2A, LocD2A.
  simpl in *.
  reflexivity.
+ destruct dist,
  (AGF.Config.eq_RobotConf_dec (ConfigD2A c (Good g)) (rcD2A (c (Good g))));
...
+ destruct dist,
  ...
  reflexivity.
+ destruct dist,
  (AGF.Config.eq_RobotConf_dec (ConfigD2A c (Byz b)) (rcD2A (c (Byz b))));
    try discriminate.
  destruct n.
  unfold ConfigD2A, rcD2A, LocD2A.
  simpl in *.
...
+ simpl in *.
  unfold LocD2A.
  destruct (DGF.Config.loc (c' (Good g))) eqn : HlocD'; try now rewrite HlocD in Hloc.
+ simpl in *.
  unfold LocD2A.
  destruct (DGF.Config.loc (c' (Good g))) eqn : HlocD'; rewrite HlocD in Hloc;
    try now simpl.
  unfold DGF.loc_eq in *.
  ...
  destruct (Rle_dec (DGF.project_p pld) (Graph.threshold eld));
    try (now apply Graph.src_compat); try now apply Graph.tgt_compat.
+ simpl in *; unfold LocD2A.
  destruct (DGF.Config.source (DGF.Config.robot_info (c' (Good g))))eqn : Hs';
    now rewrite HlocD in Hsrc.
+ simpl in *; unfold LocD2A.
  destruct (DGF.Config.source (DGF.Config.robot_info (c' (Good g)))) eqn : HlocD';
  ...
  destruct (Rle_dec (DGF.project_p pld) (Graph.threshold eld));
    try (now apply Graph.src_compat); try now apply Graph.tgt_compat.
+ unfold LocD2A.
  destruct (DGF.pgm rbg
            (AGF.Spect.from_config
               (AGF.Config.map (AGF.apply_sim sim1) (ConfigD2A c)))) eqn : Hlocpgm;
  destruct (DGF.Config.target (DGF.Config.robot_info (c' (Good g)))) as [lgt' | egt' pgt'] eqn : HtgtD';
  try (now simpl in *).
  - clear Hmi.
    simpl in *.
    destruct (AGF.Config.eq_RobotConf_dec (ConfigD2A c (Good g)) (rcD2A (c (Good g)))); try discriminate.
    destruct (DGF.pgm rbg (DGF.Spect.from_config (DGF.Config.map (DGF.apply_sim sim0) c))) eqn : Hpgm.
    rewrite Htgt.
    f_equiv.
    assert (Hiso_eq_01: AGF.Aom_eq (AGF.Active sim0) (AGF.Active sim1))
      by now rewrite HstepA.
    unfold AGF.Aom_eq in Hiso_eq_01.
    apply Hiso_eq_01.
    assert (DGF.Location.eq (DGF.Loc l0) (DGF.Loc l)).
    { assert (Hiso_eq_01: AGF.Aom_eq (AGF.Active sim0) (AGF.Active sim1))
      by now rewrite HstepA.
      unfold AGF.Aom_eq in Hiso_eq_01.
      rewrite <- Hlocpgm, <- Hpgm.
      unfold DGF.Spect.from_config, AGF.Spect.from_config.
      assert (DGF.Spect.eq ((DGF.projectS (DGF.Config.map (DGF.apply_sim sim0) c)))
                           (AGF.Config.map (AGF.apply_sim sim1) (ConfigD2A c))).
      unfold DGF.projectS, DGF.Config.map, DGF.apply_sim, DGF.projectS_loc,
      AGF.Config.map, AGF.apply_sim, ConfigD2A, LocD2A;
        simpl.
      unfold DGF.Spect.eq, DGF.View.eq, ConfigA.eq.
      intro id.
      simpl in *.
      destruct (DGF.Config.loc (c id)) eqn : Hloc_id;
        destruct (DGF.Config.source (DGF.Config.robot_info (c id))) eqn : Hsrc_id;
        destruct (DGF.Config.target (DGF.Config.robot_info (c id))) eqn : Htgt_id;
        simpl;
      ...
      destruct (Rle_dec
         (DGF.project_p
            (DGF.project_p_inv
               (Isomorphism.section (Iso.sim_T sim0) (DGF.project_p p))))
         (Graph.threshold (Isomorphism.section (Iso.sim_E sim0) e0))),
      (Rle_dec (DGF.project_p p) (Graph.threshold e0));
      ...
      try (assert (Hbound := DGF.project_p_image p);
      ...
      try (destruct n;
        ...
        specialize (Hcrois (Isomorphism.retraction (Iso.sim_T sim) x)
          ...
          apply Rnot_lt_le;
          apply (Hc H0));
          destruct (H0 (Isomorphism.section (Iso.sim_T sim0) (DGF.project_p p))
        ...
        lra);
     try (destruct n;
          rewrite <- DGF.inv_pro; try (rewrite <- (Iso.sim_bound_T sim0);
      apply DGF.project_p_image);
          rewrite <- Hthresh0;
      destruct (Rle_lt_or_eq_dec (DGF.project_p p) (Graph.threshold e0)) as [rx|ex];
      try (assumption);
    ...
    assert (Hrange := DGF.pgm_range rbg (DGF.Spect.from_config (DGF.Config.map (DGF.apply_sim sim0) c)) g ((ConfigA.loc
                (DGF.Spect.from_config (DGF.Config.map (DGF.apply_sim sim0) c)
                                       (Good g))))).
    assert (Graph.Veq (ConfigA.loc ((DGF.Spect.from_config (DGF.Config.map (DGF.apply_sim sim0) c)) (Good g))) l).
    unfold DGF.Spect.from_config, DGF.Config.map, DGF.apply_sim, DGF.projectS, DGF.projectS_loc; simpl.
    rewrite HlocD; simpl.
    ...
    rewrite Hfalse in Hpgm.
    discriminate.
  - assert (Hrange := DGF.pgm_range rbg  (AGF.Spect.from_config
                 (AGF.Config.map (AGF.apply_sim sim1) (ConfigD2A c))) g  (ConfigA.loc
                (AGF.Spect.from_config
                   (AGF.Config.map (AGF.apply_sim sim1) (ConfigD2A c)) 
                   (Good g)))).
    destruct Hrange as (lrange, (erange, (Hfalse, _))).
    unfold AGF.Spect.from_config, AGF.Config.map, AGF.apply_sim, ConfigD2A, LocD2A.
    simpl.
    
    reflexivity.
    rewrite Hfalse in Hlocpgm.
    discriminate.
+ assert (Hdelta := DGF.step_delta da g (c (Good g)) sim0 HstepD).
  destruct Hdelta as ((ldelta, Hldelta),Heq_delta).
  rewrite HlocD in Hldelta.
  now unfold DGF.Location.eq in Hldelta.
+ unfold ConfigD2A, rcD2A, LocD2A in *;
