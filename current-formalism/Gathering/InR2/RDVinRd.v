...
Definition gathered_at (pt : Loc.t) (conf : Config.t) := forall g : Names.G, conf (Good g) = pt.
...
Definition forbidden (conf : Config.t) :=
  Nat.Even N.nG /\ let m := Spec.from_config(conf) in
  exists p1 p2, ~Loc.eq p1 p2 /\ Spec.multiplicity p1 m = N.nG / 2 /\ Spec.multiplicity p2 m = N.nG / 2.

Instance forbidden_invariant : Proper (Config.eq ==> iff) forbidden.
Proof.
...
Definition solGathering (r : robogram) (d : demon) :=
  forall conf, ~forbidden conf -> exists pt : Loc.t, WillGather pt (execute r d conf).
...
Definition on_sphere c R x :=
  if Rdec_bool (Loc.dist c x) R then true else
  if Rdec_bool (Loc.dist c x) 0 then true else false.

Definition all_on_sphere c R := forallb (fun x => on_sphere c R x).

Definition robogram_pgm (s : Spec.t) : Loc.t :=
  match Spec.support (max s) with
    ...
    | _ => (* several majority stacks *)
      let '(c, R) := Loc.smallest_enclosing_sphere (Spec.support s) in
...
Proof. intros s s' Hs Hnil. apply (@PermutationA_nil _ Loc.eq _). rewrite <- Hs, Hnil. reflexivity. Qed.

Lemma support_max_singleton : forall s s' x, Spec.eq s s' ->
  Spec.support (max s) = x :: nil -> exists y, Loc.eq x y /\ Spec.support (max s') = y :: nil.
Proof. intros s s' x Heq Hs. apply (PermutationA_length1 _). rewrite <- Heq, Hs. reflexivity. Qed.

Instance on_sphere_compat : Proper (Loc.eq ==> eq ==> Loc.eq ==> eq) on_sphere.
Proof.
intros c1 c2 Hc R1 R2 HR x1 x2 Hx. unfold on_sphere.
rewrite Hc, Hx, HR. destruct (Rdec_bool (Loc.dist c2 x2) R2); trivial.
rewrite Hc, Hx. destruct (Rdec_bool (Loc.dist c2 x2) 0); trivial.
Qed.
...
Instance all_on_sphere_compat : Proper (Loc.eq ==> eq ==> PermutationA Loc.eq ==> eq) all_on_sphere.
Proof.
...
Instance robogram_compat : Proper (Spec.eq ==> Loc.eq) robogram_pgm.
Proof.
...
+ destruct (Spec.support (max s')) as [| pt' [| ? l']] eqn:Hs'.
  ...
  - destruct (Loc.smallest_enclosing_sphere (Spec.support s)) as [c1 R1] eqn:Heq1,
              (Loc.smallest_enclosing_sphere (Spec.support s')) as [c2 R2] eqn:Heq2.
    assert (Hsphere : (Loc.eq * eq)%signature (c1, R1) (c2, R2)).
    ...
    destruct (Rdec_bool (Loc.dist c2 Loc.origin) 0); try reflexivity. rewrite Hc.
    destruct (Rdec_bool (Loc.dist c2 Loc.origin) R2); trivial; reflexivity.
Qed.
...
Lemma round_simplify : forall (da : demonic_action) conf,
  Config.eq (round GatheringRobogram da conf)
         (fun id => match da.(step) id with
                      ...
                      | Some f =>
                        match id with
                          ...
                          | Good g =>
                            match Spec.support (max (Spec.from_config conf)) with
                              | nil => Loc.origin (* only happen with no robots *)
                              | pt :: nil => pt (* case 1: one majority stack *)
                              | _ => (* several majority stacks *)
                                let '(c, R) := Loc.smallest_enclosing_sphere
                                                 (Spec.support (Spec.from_config conf)) in
                                if all_on_sphere c R (Spec.support (Spec.from_config conf)) then c else
                                if Rdec_bool (Loc.dist c Loc.origin) 0 then Loc.origin else
                                if Rdec_bool (Loc.dist c Loc.origin) R then Loc.origin else c
                            end
                        end
...
Proof.
intros da conf id. unfold round. destruct (step da id) as [| obs Hobs]. reflexivity.
...
workaround: *)
assert (h_isok:∀ (g : Names.G) (conf : Config.t), Spec.is_ok (spectrum_of da g conf) conf). {
...
unfold robogram.
rewrite (lift_gp_equiv {| gp := gp conf; bp := locate_byz da |}). simpl. rewrite <- lift_gp_equiv.
...
+ rewrite <- majority_stack_Invalid_similarity in Hmaj; try exact R1_neq_R0. rewrite Hmaj.
  rewrite (lift_gp_equiv {| gp := gp conf; bp := locate_byz da |}). simpl. rewrite <- lift_gp_equiv.
  assert (Hinj : injective eq eq (fun x => 1 * (x - gp conf g))) by (apply similarity_injective; lra).
  assert (Proper (eq ==> eq) (fun x => 1 * (x - gp conf g))) by now repeat intro; subst.
  rewrite nominal_spectrum_similarity, multiset_map, M.map_support, map_length; trivial.
  assert (H0 : 1 * (gp conf g - gp conf g) = 0) by ring.
  unfold M.elt.
  destruct (beq_nat (@length R (M.support (multiset (nominal_spectrum conf)))) 3) eqn:H3.
  - rewrite nominal_spectrum_similarity, multiset_map, M.map_support; trivial.
    ...
    rewrite <- H0 at 1. rewrite (map_nth (fun x => 1 * (x - gp conf g))). field_simplify.
  ...
  - rewrite <- H0 at 1. rewrite is_extremal_similarity_invariant; try exact R1_neq_R0.
    rewrite (lift_gp_equiv {| gp := gp conf; bp := locate_byz da |}). simpl. rewrite <- lift_gp_equiv.
    destruct (is_extremal (gp conf g) (nominal_spectrum conf)). field.
    rewrite extreme_center_similarity_invariant; try exact R1_neq_R0. field_simplify.
    rewrite (lift_gp_equiv {| gp := gp conf; bp := locate_byz da |}). simpl. now rewrite <- lift_gp_equiv.
Qed.
Print Assumptions round_simplify.

Theorem nominal_spectrum_simplify : forall (da : demonic_action nG 0) conf,
  Permutation (nominal_spectrum conf) (map (gp conf) (idle da) ++ map (gp conf) (active da)).
Proof.
intros da conf.
rewrite (lift_gp_equiv conf), <- (if_ExtEq (fun g => Rdec_bool (frame da g) 0) (gp conf)) at 1.
...
Theorem nominal_spectrum_round_simplify : forall (da : demonic_action nG 0) conf,
  Permutation
    (nominal_spectrum (lift_gp (round robogram da (gp conf))))
    (* robots that are not activated this turn *)
    (map (gp conf) (idle da)
    (* robots that are activated this turn *)
    ++ map (fun g => match majority_stack (nominal_spectrum conf) with
                       | NoResult => 0
                       | Valid pt n => pt
                       | Invalid n => if beq_nat (length (M.support (multiset (nominal_spectrum conf)))) 3
                                      then List.nth 1 (sort (M.support (multiset (nominal_spectrum conf)))) 0
                                      else if is_extremal (gp conf g) (nominal_spectrum conf) then gp conf g
                                           else extreme_center (nominal_spectrum conf)
                     end)
           (active da)).
Proof.
intros da conf. rewrite round_simplify. unfold lift_gp. rewrite (if_Rdec_ExtEq 0 (frame da)).
rewrite nominal_spectrum_combineG. simpl. rewrite app_nil_r, partition_filter. reflexivity.
Qed.

(** ** Simplification of the global configuration in the three cases of the robogram **)

(** When there is a majority stack, it grows and all other stacks wither. **)
Theorem Stack_at_grow :  forall pt conf (da : demonic_action nG 0),
  Stack_at pt conf -> (M.multiplicity pt (multiset (nominal_spectrum (lift_gp (round robogram da (gp conf)))))
                     >= M.multiplicity pt (multiset (nominal_spectrum conf)))%nat.
Proof.
intros pt conf da Hstack.
...
Theorem Stack_at_wither : forall pt pt' conf (da : demonic_action nG 0), pt <> pt' ->
  Stack_at pt conf -> (M.multiplicity pt' (multiset (nominal_spectrum (lift_gp (round robogram da (gp conf)))))
                     <= M.multiplicity pt' (multiset (nominal_spectrum conf)))%nat.
Proof.
intros pt pt' conf da Hdiff Hstack.
(* we first simplify the lhs *)
rewrite <- majority_stack_spec in Hstack. destruct Hstack as [m Hstack].
...
Theorem Stack_at_forever : forall pt conf (da : demonic_action nG 0),
  Stack_at pt conf -> Stack_at pt (lift_gp (round robogram da (gp conf))).
Proof.
intros pt conf da Hstack. assert (Hs := Hstack). destruct Hs as [n [? Hconf]].
eexists. split. reflexivity. intros pt' Hdiff.
eapply le_lt_trans. apply Stack_at_wither with pt; trivial.
eapply lt_le_trans. apply Hconf. assumption.
...
Lemma round_simplify_Three : forall (da : demonic_action nG 0) conf n,
  ...
  ExtEq (round robogram da (gp conf))
        (fun g => if Rdec (frame da g) 0 then gp conf g
                  else nth 1 (sort (M.support (multiset (nominal_spectrum conf)))) 0).
Proof.
intros da conf n Hmaj H3.
...
Lemma nominal_spectrum_3_stacks : forall conf, length (M.support (multiset (nominal_spectrum conf))) = 3%nat <->
  exists pt1 pt2 pt3, exists n1 n2 n3, pt1 <> pt2 /\ pt2 <> pt3 /\ pt1 <> pt3 /\
    (0 < n1)%nat /\ (0 < n2)%nat /\ (0 < n3)%nat /\
    Permutation (nominal_spectrum conf) (alls pt1 n1 ++ alls pt2 n2 ++ alls pt3 n3).
Proof.
intro conf. split; intro H.
+ assert (Hl : exists pt1 pt2 pt3, M.support (multiset (nominal_spectrum conf)) =  pt1 :: pt2 :: pt3 :: nil).
  { destruct (M.support (multiset (nominal_spectrum conf))) as [| a [| b [| c [| d l]]]]; inversion H.
    exists a. exists b. exists c. reflexivity. } clear H.
  destruct Hl as [pt1 [pt2 [pt3 Hl]]]. exists pt1. exists pt2. exists pt3.
  exists (M.multiplicity pt1 (multiset (nominal_spectrum conf))).
  exists (M.multiplicity pt2 (multiset (nominal_spectrum conf))).
  exists (M.multiplicity pt3 (multiset (nominal_spectrum conf))).
  assert (Hdup := M.support_NoDupA (multiset (nominal_spectrum conf))).
  ...
  - change (M.In pt1 (multiset (nominal_spectrum conf))). rewrite <- M.support_spec, Hl. now left.
  - change (M.In pt2 (multiset (nominal_spectrum conf))). rewrite <- M.support_spec, Hl. now right; left.
  - change (M.In pt3 (multiset (nominal_spectrum conf))). rewrite <- M.support_spec, Hl. now do 2 right; left.
  - do 3 rewrite multiset_spec. etransitivity. apply (remove_count_occ_restore Rdec pt1).
    ...
    assert (Hin : forall x, In x (nominal_spectrum conf) -> x = pt1 \/ x = pt2 \/ x = pt3).
    { intros x Hin. cut (In x (M.support (multiset (nominal_spectrum conf)))).
        rewrite Hl. intros [| [| [|]]]; intuition. now inversion H. now apply multiset_support. }
    assert (Hrem : forall x, In x (remove Rdec pt2 (remove Rdec pt1 (nominal_spectrum conf))) -> x = pt3).
    ...
    clear Hrem Hl. induction (nominal_spectrum conf).
      reflexivity.
...
Corollary nominal_spectrum_Three : forall conf n, 
  majority_stack (nominal_spectrum conf) = Invalid n ->
  length (M.support (multiset (nominal_spectrum conf))) = 3%nat ->
  exists pt1 pt2 pt3, exists m, pt1 <> pt2 /\ pt2 <> pt3 /\ pt1 <> pt3 /\ (0 < m <= n)%nat /\
    Permutation (nominal_spectrum conf) (alls pt1 n ++ alls pt2 n ++ alls pt3 m).
Proof.
intros conf n Hmaj H3. rewrite nominal_spectrum_3_stacks in H3.
...
Theorem Three_grow : forall (da : demonic_action nG 0) conf n,
  majority_stack (nominal_spectrum conf) = Invalid n ->
  length (M.support (multiset (nominal_spectrum conf))) = 3%nat ->
  let pt := nth 1 (sort (M.support (multiset (nominal_spectrum conf)))) 0 in
  (M.multiplicity pt (multiset (nominal_spectrum (lift_gp (round robogram da (gp conf)))))
   >= M.multiplicity pt (multiset (nominal_spectrum conf)))%nat.
Proof.
intros da conf n Hmaj H3. hnf.
pose (pt := nth 1 (sort (M.support (multiset (nominal_spectrum conf)))) 0%R). fold pt.
(* We simplify the lhs *)
...
Theorem Three_wither : forall (da : demonic_action nG 0) conf n,
  majority_stack (nominal_spectrum conf) = Invalid n ->
  length (M.support (multiset (nominal_spectrum conf))) = 3%nat ->
  forall pt, pt <> nth 1 (sort (M.support (multiset (nominal_spectrum conf)))) 0 ->
  (M.multiplicity pt (multiset (nominal_spectrum (lift_gp (round robogram da (gp conf)))))
   <= M.multiplicity pt (multiset (nominal_spectrum conf)))%nat.
Proof.
intros da conf n Hmaj H3 pt Hdiff.
(* we first simplify the lhs *)
rewrite nominal_spectrum_round_simplify, Hmaj, H3, <- beq_nat_refl.
rewrite multiset_app, M.union_spec.
(* Same thing for the rhs *)
...
Lemma round_simplify_Generic : forall (da : demonic_action nG 0) conf n x t,
  majority_stack (nominal_spectrum conf) = Invalid n ->
  range x t (nominal_spectrum conf) -> In x (nominal_spectrum conf) -> In t (nominal_spectrum conf) ->
  length (M.support (multiset (nominal_spectrum conf))) <> 3%nat ->
  ExtEq (round robogram da (gp conf))
        (fun g => if Rdec (frame da g) 0 then gp conf g else
                  if Rdec (gp conf g) x then x else if Rdec (gp conf g) t then t else (x + t) / 2).
Proof.
intros da conf n min max Hmaj Hrange Hmin Hmax H3.
rewrite round_simplify; trivial. intro g. Rdec_full. reflexivity.
rewrite Hmaj. rewrite <- beq_nat_false_iff in H3. rewrite H3.
assert (Hextreme : forall pt, is_extremal pt (nominal_spectrum conf) = true <-> pt = min \/ pt = max).
{ intro pt. unfold is_extremal. repeat Rdec_full; split; intro H; try reflexivity || discriminate H.
  + clear H. left. rewrite Heq. now apply (@Generic_min (nominal_spectrum conf) n min max).
  + clear H. right. rewrite Heq. now apply (@Generic_max (nominal_spectrum conf) n min max).
  + exfalso. destruct H.
    - apply Hneq0. subst pt. symmetry. now apply (@Generic_min (nominal_spectrum conf) n min max).
    - apply Hneq1. subst pt. symmetry. now apply (@Generic_max (nominal_spectrum conf) n min max). }
destruct (is_extremal (gp conf g) (nominal_spectrum conf)) eqn:Hext.
  rewrite Hextreme in Hext. destruct Hext; repeat Rdec_full; assumption || contradiction.
  repeat Rdec_full.
  - apply (@or_introl _ (gp conf g = max)) in Heq. rewrite <- Hextreme in Heq.
    rewrite Heq in Hext. discriminate Hext.
  - apply (@or_intror (gp conf g = min)) in Heq. rewrite <- Hextreme in Heq.
    rewrite Heq in Hext. discriminate Hext.
...
Theorem Generic_grow : forall (da : demonic_action nG 0) conf n x t,
  majority_stack (nominal_spectrum conf) = Invalid n ->
  range x t (nominal_spectrum conf) -> In x (nominal_spectrum conf) -> In t (nominal_spectrum conf) ->
  length (M.support (multiset (nominal_spectrum conf))) <> 3%nat ->
  (M.multiplicity ((x + t) / 2) (multiset (nominal_spectrum conf))
   <= M.multiplicity ((x + t) / 2) (multiset (nominal_spectrum (lift_gp (round robogram da (gp conf))))))%nat.
Proof.
intros da conf n x t Hmaj Hrange Hx Ht H3. hnf.
pose (pt := nth 1 (sort (M.support (multiset (nominal_spectrum conf)))) 0%R). fold pt.
(* We simplify the lhs *)
...
(* Non activated robots are in the same locations in both configurations so we can simplify them out *)
apply plus_le_compat. reflexivity.
(* Extremal robots are in the same locations in both configurations so we can simplify them out *)
(*
SearchAbout Proper M.multiplicity M.eq.
SearchAbout Proper multiset.
SearchAbout Proper map Permutation.
Check (if_ExtEq (fun g => is_extremal (gp conf g) (nominal_spectrum conf)) (gp conf)). (* BUG rewrite? *)
*)
...
Theorem Generic_wither : forall (da : demonic_action nG 0) conf n x t,
  majority_stack (nominal_spectrum conf) = Invalid n ->
  range x t (nominal_spectrum conf) -> In x (nominal_spectrum conf) -> In t (nominal_spectrum conf) ->
  length (M.support (multiset (nominal_spectrum conf))) <> 3%nat ->
  forall pt, pt <> ((x + t ) / 2) ->
  (M.multiplicity pt (multiset (nominal_spectrum (lift_gp (round robogram da (gp conf)))))
   <= M.multiplicity pt (multiset (nominal_spectrum conf)))%nat.
Proof.
intros da conf n x t Hmaj Hrange Hx Ht H3 pt Hdiff.
(* We simplify the lhs *)
rewrite <- beq_nat_false_iff in H3.
rewrite nominal_spectrum_round_simplify, Hmaj, H3, multiset_app, M.union_spec.
(* Same thing for the rhs *)
...
Theorem Stack_at_meeting : forall conf pt (d : demon nG 0) k,
  kFair k d -> Stack_at pt conf -> WillGather pt (execute robogram d (gp conf)).
Proof.
Admitted.
...
Lemma forbidden_elements_perm : forall conf,
  forbidden conf <-> exists pt1 pt2, pt1 <> pt2 /\ Permutation (M.elements (multiset (nominal_spectrum conf)))
                                                              ((pt1, div2 nG) :: (pt2, div2 nG) :: nil).
Proof.
intro conf. split; intros [pt1 [pt2 [Hneq Hconf]]]; exists pt1; exists pt2; split; trivial.
+ rewrite Permutation_eq_pair_equiv, Hconf, multiset_app, multiset_alls, multiset_alls, M.add_union_singleton.
  ...
  - intros [x n]. rewrite M.elements_add. simpl. split; intro H.
      ...
      rewrite M.singleton_spec. inversion_clear H.
        left. destruct H0. hnf in *; simpl in *. subst. repeat split.
          ...
          now apply half_size_conf.
        right. inversion_clear H0.
          split.
            destruct H as [H _]. hnf in H. simpl in H. subst. now auto.
            rewrite M.elements_singleton. now left. now apply half_size_conf.
          inversion H.
+ assert (Hin : forall xn, InA M.eq_pair xn (M.elements (multiset (nominal_spectrum conf)))
                           <-> (fst xn = pt1 \/ fst xn = pt2) /\ snd xn = div2 nG).
  { intros [x n]. rewrite Permutation_eq_pair_equiv in Hconf. rewrite Hconf.
    split; intro H.
    + inversion H.
  ...
  assert (Hmul1 : M.multiplicity pt1 (multiset (nominal_spectrum conf)) = div2 nG).
  { apply proj1 with (div2 nG > 0)%nat. rewrite <- (M.elements_spec (pt1, div2 nG)).
    rewrite Hin. now split; try left. }
  assert (Hmul2 : M.multiplicity pt2 (multiset (nominal_spectrum conf)) = div2 nG).
...
Corollary forbidden_elements : forall conf,
  forbidden conf <-> exists pt1 pt2, pt1 <> pt2
                    /\ M.elements (multiset (nominal_spectrum conf)) = (pt1, div2 nG) :: (pt2, div2 nG) :: nil.
Proof.
intro conf. rewrite forbidden_elements_perm. split; intros [pt1 [pt2 [Hdiff H]]].
...
Corollary forbidden_multiplicity : forall conf,
  forbidden conf <-> exists pt1 pt2, pt1 <> pt2
              /\ M.multiplicity pt1 (multiset (nominal_spectrum conf)) = div2 nG
              /\ M.multiplicity pt2 (multiset (nominal_spectrum conf)) = div2 nG
              /\ forall pt, pt <> pt1 -> pt <> pt2 -> M.multiplicity pt (multiset (nominal_spectrum conf)) = 0%nat.
Proof.
intro conf. split; intros [pt1 [pt2 [Hdiff H]]].
...
+ rewrite forbidden_elements_perm. exists pt1. exists pt2. split; trivial.
  ...
  - intros [x n]. destruct H as [Hin1 [Hin2 Hin]]. destruct (Rdec x pt1).
      subst. rewrite M.elements_spec. simpl. rewrite Hin1. split; intro H.
        now left.
        inversion_clear H.
          destruct H0 as [_ H]. hnf in H; simpl in H. subst n. split. reflexivity. now apply half_size_conf.
      ...
      destruct (Rdec x pt2).
        subst. rewrite M.elements_spec. simpl. rewrite Hin2. split; intro H.
          ...
          inversion_clear H.
            ...
            inversion_clear H0.
              destruct H as [_ H]. hnf in H; simpl in H. subst n. split. reflexivity. now apply half_size_conf.
              now inversion H.
        split; intro H.
...
Lemma forbidden_even : forall conf, forbidden conf -> NPeano.Even nG.
Proof.
intros conf [pt1 [pt2 [Hdiff Hperm]]]. exists (div2 nG).
rewrite <- plus_0_r at 1. rewrite <- (nominal_spectrum_length conf).
rewrite Hperm, app_length, alls_length, alls_length. omega.
Qed.

Lemma Stack_at_forbidden : forall pt conf, Stack_at pt conf -> ~forbidden conf.
Proof.
intros pt conf [n [Hstack Hother]] [pt1 [pt2 [Hdiff Habs]]]. revert Hstack Hother.
setoid_rewrite Habs.
...
Theorem never_forbidden : forall (da : demonic_action nG 0) conf,
  ~forbidden conf -> ~forbidden (lift_gp (round robogram da conf.(gp))).
Proof.
intros da conf Hok.
...
+ (* We express more directly the configuration after one round *)
  destruct (beq_nat (length (M.support (multiset (nominal_spectrum conf)))) 3) eqn:Hn.
  - (* 2) There are exactly three stacks *)
    ...
    assert (Hsup : Permutation (M.support (multiset ((nominal_spectrum conf)))) (pt1 :: pt2 :: pt3 :: nil)).
    ...
    assert (Hext : ExtEq (fun g => if Rdec (frame da g) 0 then gp conf g
                                   else nth 1 (sort (M.support (multiset (nominal_spectrum conf)))) 0)
                         (fun g => if Rdec (frame da g) 0 then gp conf g
...
Theorem Will_stack : forall da conf, ~forbidden conf -> (forall pt, ~Stack_at pt conf) ->
  Stack_at (lift_gp (round robogram da conf.(gp))).
Proof.
intros da conf Hok Hyet.
rewrite <- majority_stack_spec in Hyet.
assert (exists n, majority_stack (nominal_spectrum conf) = Invalid n).
{ destruct (majority_stack (nominal_spectrum conf)) eqn:Hconf.
  - rewrite majority_stack_NoResult_spec in Hconf. rewrite nominal_spectrum4 in Hconf. discriminate Hconf.
  - elim Hyet. exists l. exists n. reflexivity.
  - exists n. reflexivity. }
destruct H as [n Hn]. clear Hyet.
(* We express more directly the configuration after one round *)
assert (Heq : ExtEq (round robogram da (gp conf))
             (fun g => if is_extremal (gp conf g) (nominal_spectrum conf)
                       then gp conf g else extreme_center (nominal_spectrum conf))).
{ etransitivity. apply round_simplify. assumption.
  intro g. destruct (Rdec (frame da g) 0) as [| Hframe]. now elim (active da g).
  now rewrite Hn. }
assert (Hl : length (sort (nominal_spectrum conf)) = 4%nat).
{ rewrite (Permutation_length (symmetry (Permuted_sort _))). now rewrite nominal_spectrum4. }
assert (Hperm := Permuted_sort (nominal_spectrum conf)).
assert (Hsort := Sorted_sort (nominal_spectrum conf)).
destruct (sort (nominal_spectrum conf)) as [| x [| y [| z [| t [| ]]]]] eqn:Hs; try discriminate Hl. clear Hl.
...
assert (Hperm2 : Permutation (nominal_spectrum (lift_gp (round robogram da (gp conf))))
                             (x :: (x + t) / 2 :: (x + t) / 2 :: t :: nil)).
{ apply (@nominal_spectrum_simplify da conf n x t Hok Hn).
...
exists (extreme_center (nominal_spectrum conf)). exists 2%nat. split.
