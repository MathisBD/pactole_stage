...
(* FIXME: These three definitions: gathered_at, gather and WillGather
   ...
   module? We could even add a generic notion of bivalent
   configurations. *)
...
(** When all robots are on two towers of the same height,
    ...
    Therefore, we define these configurations as [bivalent]. *)
Definition bivalent (config : Config.t) :=
...
(** [ValidsolGathering r d] means that any possible (infinite)
    execution, generated by demon [d] and robogram [r] and any intial
    configuration not [bivalent], will *eventually* be [Gather]ed.
    This is the statement used for the correctness proof of the algorithm. *)
Definition ValidSolGathering (r : robogram) (d : demon) :=
  forall config, ~bivalent config -> exists pt : R2.t, willGather pt (execute r d config).


Instance bivalent_compat : Proper (Config.eq ==> iff) bivalent.
Proof.
...
Lemma not_bivalent_no_majority_size : forall conf,
...
(** ***  A bivalent configuration cannot appear  **)
...
Theorem never_bivalent : forall da conf, ~bivalent conf -> ~bivalent (round gatherR2 da conf).
Proof.
intros da conf Hok.
...
- get_case conf.
  ...
  * intro Habs.
    ...
    { assert (Hpt'_in : Spect.In pt' (!! conf)).
      ...
      assert (Hle := not_bivalent_no_majority_size Hmaj Hok).
...
Proof.
  intros conf ptx pty Hbivalent hmax Hclean HonSEC.
  ...
  - rewrite <- Spect.size_spec. now apply not_bivalent_no_majority_size.
Qed.
...
Proof.
  intros da conf Hbivalent Hcleandiam Hmaj'.
  ...
  assert (Hincl := incl_next da conf).
  assert (Htarget : target (!!conf) = R2.middle ptx pty) by (apply diameter_target; auto).
  assert (Hperm := @diameter_clean_support conf ptx pty Hbivalent Hmaj Hclean Hsec).
  ...
  { apply (NoDupA_inclA_length_PermutationA _).
    ...
    - cbn. rewrite <- Spect.size_spec. now apply not_bivalent_no_majority_size, never_bivalent. }
...
Proof.
intros da config ptx pty Hbivalent Hmaj Hclean Hsec.
...
Proof.
intros da conf Hbivalent [Hmaj [ptx [pty [ptz Hsec]]]].
...
- right.
  ...
  clear Hmax' e e0 l.
  assert (Hbivalent' : ~bivalent (round gatherR2 da conf)) by now apply never_bivalent.
  assert (Hlen' : Spect.size (!! (round gatherR2 da conf)) >= 3) by now apply not_bivalent_no_majority_size.
...
Theorem round_lt_config : forall da conf,
  ~bivalent conf -> moving gatherR2 da conf <> nil ->
  lt_config (round gatherR2 da conf) conf.
Proof.
  intros da conf Hbivalent Hmove. unfold lt_config.
  unfold measure at 2.
  ...
  - (* Computing the SEC *)
    ...
    + assert (Hle := no_Majority_on_SEC_length Hmaj).
      ...
      * (* Diameter case *)
        ...
        assert (Hperm := diameter_clean_support Hbivalent Hmaj Hclean Hsec).
        destruct (clean_diameter_next_maj_or_diameter da Hbivalent Hmaj Hclean Hsec)
        ...
        -- (* Still in a diameter case after one round *)
           ...
           assert (Htarget' := diameter_next_target_same Hbivalent Hcase Hmaj').
      ...
      * (* Triangle cases *)
        get_case conf.
        assert (HnextSEC := triangle_next_maj_or_diameter_or_triangle da Hbivalent Hcase).
...
(** Correctness proof: given a non-gathered, non bivalent configuration, then some robot will move some day. *)
...
Theorem Fair_FirstMove : forall d, Fair d ->
  forall config id, ~bivalent config -> ~gathered_at (config id) config -> FirstMove gatherR2 d config.
Proof.
