...
Lemma canonical_form : forall a b c x, a <> 0 ->
  (a * x² + b * x + c = 0 <-> (x + b / (2 * a))² - (b² - 4 * a * c) / (2 * a)² = 0).
Proof.
intros a b c x Ha. split; intro Heqn.
- apply (Rmult_eq_reg_r a); trivial; []. rewrite Rmult_0_l. rewrite <- Heqn. unfold Rsqr. now field.
- apply (Rmult_eq_reg_r (/a)); try (now apply Rinv_neq_0_compat); [].
  rewrite Rmult_0_l. rewrite <- Heqn. unfold Rsqr. now field.
Qed.

Lemma second_order_equation_Delta_pos : forall a b c, a <> 0 -> 0 <= b² - 4 * a * c ->
  forall x, (a * x² + b * x + c = 0 <-> x = (- b + sqrt (b² - 4 * a * c)) / (2 * a) \/
                                        x = (- b - sqrt (b² - 4 * a * c)) / (2 * a)).
Proof.
intros a b c Ha HΔ x. split; intro Heqn.
+ rewrite (canonical_form b c x Ha) in Heqn.
  rewrite <- (Rsqr_sqrt _ HΔ), <- R_sqr.Rsqr_div in Heqn; try lra; [].
  rewrite <- R_sqr.Rsqr_minus_plus in Heqn. apply Rmult_integral in Heqn.
  destruct Heqn; left + right; unfold Rsqr in *; lra.
+ destruct Heqn as [Heqn | Heqn]; subst; unfold Rsqr; field_simplify; trivial; [|].
  - replace (Rpow_def.pow (sqrt (b * b - 4 * a * c)) 2) with (sqrt (b * b - 4 * a * c))² by (unfold Rsqr; ring).
    rewrite Rsqr_sqrt; try now unfold Rsqr in *. now field.
  - replace (Rpow_def.pow (sqrt (b * b - 4 * a * c)) 2) with (sqrt (b * b - 4 * a * c))² by (unfold Rsqr; ring).
    rewrite Rsqr_sqrt; try now unfold Rsqr in *. now field.
Qed.

Lemma second_order_equation_Delta_null : forall a b c, a <> 0 -> b² - 4 * a * c = 0 ->
  forall x, a * x² + b * x + c = 0 <-> x = - b / (2 * a).
Proof.
intros a b c Ha HΔ x. rewrite second_order_equation_Delta_pos; auto with real.
rewrite HΔ, sqrt_0. now intuition; subst; try left; field.
Qed.

Lemma second_order_equation_Delta_neg : forall a b c, a <> 0 -> b² - 4 * a * c < 0 ->
  forall x, ~a * x² + b * x + c = 0.
Proof.
intros a b c Ha HΔ x Heqn.
rewrite (canonical_form b c x Ha) in Heqn.
assert (Heqn' : (x + b / (2 * a))² = (b² - 4 * a * c) / (2 * a)²) by lra.
apply (Rlt_irrefl 0). apply Rle_lt_trans with (x + b / (2 * a))².
- apply Rle_0_sqr.
- assert (0 < (2 * a)²) by (apply Rlt_0_sqr; lra).
  rewrite Heqn'. unfold Rdiv. erewrite <- Rmult_0_l. apply Rmult_lt_compat_r; trivial. now apply Rinv_0_lt_compat.
Qed.

Corollary two_solutions_Delta_pos : forall a b c x1 x2, a <> 0 ->
  a * x1² + b * x1 + c = 0 -> a * x2² + b * x2 + c = 0 -> x1 <> x2 ->
  0 < b² - 4 * a * c.
Proof.
intros a b c x1 x2 Ha Hx1 Hx2 Hneq.
destruct (total_order_T 0 (b² - 4 * a * c)) as [[Hlt | Hnull] | Hgt].
- assumption.
- symmetry in Hnull. elim Hneq.
  rewrite second_order_equation_Delta_null in Hx1, Hx2; trivial; [].
  congruence.
- apply second_order_equation_Delta_neg in Hx1; intuition.
Qed.

Corollary no_solution_Delta_neg : forall a b c, a <> 0 ->
  (forall x, a * x² + b * x + c <> 0) -> b² - 4 * a * c < 0.
Proof.
intros a b c Ha Hx.
destruct (total_order_T 0 (b² - 4 * a * c)) as [[Hlt | Hnull] | Hgt].
- elim (Hx ((- b + sqrt (b² - 4 * a * c)) / (2 * a))).
  rewrite second_order_equation_Delta_pos; auto with real.
- elim (Hx (- b / (2 * a))).
  rewrite second_order_equation_Delta_null; auto with real.
- assumption.
Qed.

(* The intersection of a line and a circle contains at most two points. *)
Lemma line_circle_intersection_half : forall A B circ, ~R2.eq B A ->
  let c := center circ in
  let r := radius circ in
  let a := snd A - snd B in
  let b := fst B - fst A in
  let k := snd A * fst B - fst A * snd B in
  let k' := k - a * fst c - b * snd c in
  let Δ := r² * (a² + b²) - k'² in
  0 < Δ ->
  forall pt, colinear (B - A) (pt - A) -> on_circle circ pt = true -> ~R2.eq pt A ->
                   pt = R2.add c ((a * k' - b * sqrt Δ) / (a² + b²), (b * k' + a * sqrt Δ) / (a² + b²))
                \/ pt = R2.add c ((a * k' + b * sqrt Δ) / (a² + b²), (b * k' - a * sqrt Δ) / (a² + b²)).
Proof.
intros A B circ HneqAB c r a b k k' Δ HΔ_pos pt HAB_pt Hc_pt HneqApt.
rewrite on_circle_true_iff in *.
fold r c in Hc_pt.
pose (x := fst pt - fst c).
pose (y := snd pt - snd c).
(* Let us define the equation of the line. *)
assert (Heqn_pt : (snd A - snd B) * fst pt + (fst B - fst A) * snd pt = snd A * fst B - fst A * snd B).
{ rewrite <- R2sub_origin in HneqApt, HneqAB. apply colinear_decompose in HAB_pt; trivial; [].
  pose (nApt := R2norm (pt- A)). rewrite <- R2norm_0 in HneqAB. fold nApt in HAB_pt.
  rewrite <- (R2.mul_1 (unitary _)), <- (Rinv_l _ HneqAB) in HAB_pt.
  do 2 rewrite R2.mul_morph, <- Rmult_assoc in HAB_pt.
  setoid_rewrite <- R2.mul_morph in HAB_pt. rewrite <- unitary_id in HAB_pt.
  rewrite <- R2norm_0 in HneqApt. fold nApt in HneqApt.
  destruct HAB_pt as [HAB_pt | HAB_pt].
  + pose (nAB := R2norm (B - A)). fold nAB in HneqAB, HAB_pt.
    destruct A as [xA yA], B as [xB yB], pt as [xpt ypt]. unfoldR2 in HAB_pt. simpl in HAB_pt. simpl.
    replace (yA - yB) with (-(yB - yA)) by ring. injection HAB_pt. intros Hy Hx.
    apply (f_equal (Rmult (xB - xA))) in Hy.
    replace ((xB-xA) * (nApt * / nAB * (yB + - yA))) with ((nApt * / nAB) * (xB + -xA) * (yB-yA)) in Hy by ring.
    rewrite <- Hx in Hy. lra.
  + pose (nAB := R2norm (B - A)). fold nAB in HneqAB, HAB_pt.
    destruct A as [xA yA], B as [xB yB], pt as [xpt ypt]. unfoldR2 in HAB_pt. simpl in HAB_pt. simpl.
    replace (yA - yB) with (-(yB - yA)) by ring. injection HAB_pt. intros Hy Hx.
    apply (f_equal (Rmult (xB - xA))) in Hy.
    replace ((xB-xA) * (-nApt * /nAB * (yB + - yA))) with ((-nApt * /nAB) * (xB + -xA) * (yB-yA)) in Hy by ring.
    rewrite <- Hx in Hy. lra. }
fold a b k in Heqn_pt. fold x y in Hc_pt.
(* Put the circle at origin by a variable change. *)
assert (0 <= r). { rewrite <- Hc_pt. apply R2.dist_pos. }
rewrite square_dist_equiv, square_dist_simpl in Hc_pt; trivial; [].
assert (Heqn_pt' : a * x + b * y = k'). { unfold x, y, k'. lra. }
assert (Hab : a <> 0 \/ b <> 0).
{ destruct (Rdec a 0), (Rdec b 0); auto; []. subst.
elim HneqAB.  destruct A, B; simpl in *. hnf. f_equal; lra. }
(* To inject the line equation into the circle one, we need to make a division,
   hence two possibilities for the non null coefficient. *)
destruct Hab.
+ assert (Hab2 : a² + b² <> 0). { assert (Hleb := Rle_0_sqr b). assert (Hlta := Rlt_0_sqr a ltac:(easy)). lra. }
  assert (Ha2_pos : 0 < 4 * a²). { replace (4 * a²) with (2 * a)² by (unfold Rsqr; ring). apply Rlt_0_sqr. lra. }
  assert (HΔ : (-2 * b * k')² - 4 * (a² + b²) * (k'² - a²*r²) = 4 * a² * Δ) by (unfold Δ, Rsqr; ring).
  assert (Hx : x = (k' - b * y) / a). { apply (Rmult_eq_reg_l a); trivial; []. field_simplify; trivial; []. lra. }
  assert (Hy : y = (b * k' + Rabs a * sqrt Δ) / (a² + b²) \/ y = (b * k' - Rabs a * sqrt Δ) / (a² + b²)).
  { fold x y in Hc_pt. rewrite Hx in Hc_pt.
    assert (Hc_pt' : (k' - b * y)² + a² * y² = a² * r²). { rewrite <- Hc_pt. unfold Rsqr. now field. }
    assert (Heqn : (a² + b²) * y² - 2 * b * k' * y + k'² - a² * r² = 0).
    { rewrite <- Hc_pt'. unfold Rsqr. field. }
    replace ((a² + b²) * y² - 2 * b * k' * y + k'² - a² * r²)
      with ((a² + b²) * y² + (- 2 * b * k') * y + (k'² - a² * r²)) in Heqn by ring.
    rewrite second_order_equation_Delta_pos in Heqn; trivial; [|].
    - rewrite HΔ in Heqn. replace (sqrt (4 * a² * Δ)) with (2 * Rabs a * sqrt Δ) in Heqn.
      * destruct Heqn as [Heqn | Heqn]; [left | right]; rewrite Heqn; unfold Rsqr; now field.
      * replace (4 * a²) with (2 * a)² by (unfold Rsqr; ring).
        assert (Ha2 := Rle_0_sqr (2 * a)).
        rewrite sqrt_mult; trivial; [|].
        + rewrite sqrt_Rsqr_abs, Rabs_mult, (Rabs_pos_eq 2); lra.
        + now apply Rlt_le.
    - rewrite HΔ. apply Rlt_le. now apply Rmult_lt_0_compat. }
  assert (Hy' : y = (b * k' + a * sqrt Δ) / (a² + b²) \/ y = (b * k' - a * sqrt Δ) / (a² + b²)).
  { destruct (Rlt_le_dec a 0) as [Ha | Ha].
    - rewrite Rabs_left in Hy; trivial; []. lra.
    - rewrite Rabs_right in Hy; trivial; []. lra. }
  clear Hy. rename Hy' into Hy.
  (* Depending on the value of y, we can compute the value of x. *)
  destruct Hy as [Hy | Hy].
  - left. rewrite Hy in Hx. unfold x, y in Hx, Hy.
    destruct pt, c. simpl in *. f_equal; try lra.
    replace r0 with (r2 + (r0 - r2)) by ring. f_equal. rewrite Hx.
    unfold Rsqr. field. unfold Rsqr in *; auto.
  - right. rewrite Hy in Hx. unfold x, y in Hx, Hy.
    destruct pt, c. simpl in *. f_equal; try lra; [].
    replace r0 with (r2 + (r0 - r2)) by ring. f_equal. rewrite Hx.
    unfold Rsqr. field. unfold Rsqr in *; auto.
+ (* Same proof when b <> 0 intead of a <> 0. *)
  assert (Hab2 : a² + b² <> 0). { assert (Hlea := Rle_0_sqr a). assert (Hltb := Rlt_0_sqr b ltac:(easy)). lra. }
  assert (Ha2_pos : 0 < 4 * b²). { replace (4 * b²) with (2 * b)² by (unfold Rsqr; ring). apply Rlt_0_sqr. lra. }
  assert (HΔ : (-2 * a * k')² - 4 * (a² + b²) * (k'² - b²*r²) = 4 * b² * Δ) by (unfold Δ, Rsqr; ring).
  assert (Hy : y = (k' - a * x) / b). { apply (Rmult_eq_reg_l b); trivial; []. field_simplify; trivial; []. lra. }
  assert (Hx : x = (a * k' + Rabs b * sqrt Δ) / (a² + b²) \/ x = (a * k' - Rabs b * sqrt Δ) / (a² + b²)).
  { fold x y in Hc_pt. rewrite Hy in Hc_pt.
    assert (Hc_pt' : (k' - a * x)² + b² * x² = b² * r²). { rewrite <- Hc_pt. unfold Rsqr. now field. }
    assert (Heqn : (a² + b²) * x² - 2 * a * k' * x + k'² - b² * r² = 0).
    { rewrite <- Hc_pt'. unfold Rsqr. field. }
      replace ((a² + b²) * x² - 2 * a * k' * x + k'² - b² * r²)
        with ((a² + b²) * x² + (- 2 * a * k') * x + (k'² - b² * r²)) in Heqn by ring.
      rewrite second_order_equation_Delta_pos in Heqn; trivial; [|].
      - rewrite HΔ in Heqn. replace (sqrt (4 * b² * Δ)) with (2 * Rabs b * sqrt Δ) in Heqn.
        * destruct Heqn as [Heqn | Heqn]; [left | right]; rewrite Heqn; unfold Rsqr; now field.
        * replace (4 * b²) with (2 * b)² by (unfold Rsqr; ring).
          assert (Hb2 := Rle_0_sqr (2 * b)).
          rewrite sqrt_mult; trivial; [|].
          + rewrite sqrt_Rsqr_abs, Rabs_mult, (Rabs_pos_eq 2); lra.
          + now apply Rlt_le.
      - rewrite HΔ. apply Rlt_le. now apply Rmult_lt_0_compat. }
    assert (Hx' : x = (a * k' + b * sqrt Δ) / (a² + b²) \/ x = (a * k' - b * sqrt Δ) / (a² + b²)).
    { destruct (Rlt_le_dec b 0) as [Hb | Hb].
      - rewrite Rabs_left in Hx; trivial; []. lra.
      - rewrite Rabs_right in Hx; trivial; []. lra. }
    clear Hx. rename Hx' into Hx.
    (* Depending on the value of y, we can compute the value of x. *)
    destruct Hx as [Hx | Hx].
    - right. rewrite Hx in Hy. unfold x, y in Hx, Hy.
      destruct pt, c. simpl in *. f_equal; try lra; [].
      replace r1 with (r3 + (r1 - r3)) by ring. f_equal. rewrite Hy.
      unfold Rsqr. field. unfold Rsqr in *; auto.
    - left. rewrite Hx in Hy. unfold x, y in Hx, Hy.
      destruct pt, c. simpl in *. f_equal; try lra.
      replace r1 with (r3 + (r1 - r3)) by ring. f_equal. rewrite Hy.
      unfold Rsqr. field. unfold Rsqr in *; auto.
Qed.

Lemma line_circle_intersection : forall A B circ, ~R2.eq A B ->
  let c := center circ in
  let r := radius circ in
  let a := snd A - snd B in
  let b := fst B - fst A in
  ...
  let Δ := r² * (a² + b²) - k'² in
  0 < Δ ->
  forall pt, colinear (B - A) (pt - A) -> on_circle circ pt = true ->
                   pt = R2.add c ((a * k' - b * sqrt Δ) / (a² + b²), (b * k' + a * sqrt Δ) / (a² + b²))
                \/ pt = R2.add c ((a * k' + b * sqrt Δ) / (a² + b²), (b * k' - a * sqrt Δ) / (a² + b²)).
Proof.
intros A B circ HneqAB c r a b k k' Δ HΔ_pos pt HAB_pt Hc_pt.
destruct (R2.eq_dec pt A) as [HeqA | HneqA].
* assert (HneqBpt : ~R2.eq pt B) by now rewrite HeqA.
  rewrite <- colinear_opp_compat_l, <- colinear_add in HAB_pt.
  replace (- (B - A))%R2 with (A - B)%R2 in HAB_pt by (destruct A, B; simpl; f_equal; ring).
  replace (A - B + (pt - A))%R2 with (pt - B)%R2 in HAB_pt by (destruct pt, A, B; simpl; f_equal; ring).
  unfold Δ, k', k, a, b in HΔ_pos. setoid_rewrite R_sqr.Rsqr_neg in HΔ_pos at -1.
  replace (- (snd A - snd B)) with (snd B - snd A) in HΔ_pos by (destruct A, B; simpl; f_equal; ring).
  replace (- (fst B - fst A)) with (fst A - fst B) in HΔ_pos by (destruct A, B; simpl; f_equal; ring).
  replace (- (snd A * fst B - fst A * snd B - (snd A - snd B) * fst c - (fst B - fst A) * snd c))
    with (snd B * fst A - fst B * snd A - (snd B - snd A) * fst c - (fst A - fst B) * snd c)
    in HΔ_pos by (destruct A, B; simpl; f_equal; ring).
  assert (Heq := @line_circle_intersection_half B A circ HneqAB HΔ_pos pt HAB_pt Hc_pt HneqBpt).
  unfold Δ, k', k', b, a, r, c.
  destruct Heq as [Heq | Heq]; rewrite Heq.
  + admit.
  + admit.
* assert (~R2.eq B A) by congruence.
  now apply line_circle_intersection_half.
Admitted.

Lemma line_circle_intersection_two_solutions_Delta_pos : forall A B circ, ~R2.eq A B ->
  on_circle circ A = true -> on_circle circ B = true ->
  let c := center circ in
  let r := radius circ in
  let a := snd A - snd B in
  let b := fst B - fst A in
...
(* The intersection of a line and a circle contains at most two points. *)
Lemma line_circle_intersection_at_most_two : forall A B c,
  ~R2.eq A B -> on_circle c A = true -> on_circle c B = true ->
  forall pt, colinear (B - A) (pt - A) -> on_circle c pt = true -> R2.eq pt A \/ R2.eq pt B.
Proof.
intros A B circ HneqAB HcA HcB pt HAB_pt Hc_pt.
assert (HΔ_pos := line_circle_intersection_two_solutions_Delta_pos circ HneqAB HcA HcB).
assert (HcolA : colinear (B - A) (A - A)).
{ rewrite R2.add_opp. apply colinear_origin_r. }
assert (HcolB : colinear (B - A) (B - A)) by reflexivity.
assert (HeqA := line_circle_intersection circ HneqAB HΔ_pos A HcolA HcA).
assert (HeqB := line_circle_intersection circ HneqAB HΔ_pos B HcolB HcB).
assert (Heq_pt := line_circle_intersection circ HneqAB HΔ_pos pt HAB_pt Hc_pt).
destruct HeqA as [HeqA | HeqA], HeqB as [HeqB | HeqB].
- elim HneqAB. congruence.
- rewrite HeqB. rewrite HeqA at 1. tauto.
- rewrite HeqB. rewrite HeqA at 1. tauto.
- elim HneqAB. congruence.
Qed.

(** The vectors generated by three points on a circle cannot be colinear.
    The proof relies on the fact that all three points are on the same line
    and that the intersection of a circle and a line can have at most two points. *)
Corollary circle_not_colinear : forall pt1 pt2 pt3 circ,
...
destruct (@line_circle_intersection_at_most_two pt2 pt1 circ) with pt3 as [Heq | Heq]; trivial.
- congruence.
- rewrite <- colinear_opp_compat_l, R2.opp_distr_add, R2.opp_opp, R2.add_comm. now symmetry.
