...
Module AGF (Graph : GraphDef)(N : Size)(Names : Robots(N))(LocationA : LocationADef(Graph))(ConfigA : Configuration (LocationA)(N)(Names))(Import Iso : Iso(Graph) (LocationA)).
    ...
    fun id =>
      ...
      | Active sim => (* g is activated with similarity [sim (conf g)] and move ratio [mv_ratio] *)
        ...
        | Good g =>
          ...
          let target := (sim⁻¹).(Iso.sim_V) (r (Spect.from_config local_conf)) in
          {| Config.loc := pos ; 
