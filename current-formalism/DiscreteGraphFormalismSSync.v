...
Qed.
    ...
    match da.(step) id with (** first see whether the robot is activated *)
      ...
      | Some sim => (** g is activated and [sim (conf g)] is its similarity *)
        ...
        | Good g =>
          ...
          let target := ((sim rconf)⁻¹).(Iso.sim_V) local_target in (* config expressed in the frame of g *)
          {| Config.loc := target;
  ...
  Lemma LocallyFairForOne_compat_aux : forall g d1 d2,
    deq d1 d2 -> LocallyFairForOne g d1 -> LocallyFairForOne g d2.
  Proof.
