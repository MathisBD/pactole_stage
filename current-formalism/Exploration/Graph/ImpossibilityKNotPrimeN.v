...
Lemma confP_1 : forall idg g0: Names.G, exists g2:Names.G,
      Loc.eq (create_confP idg)
             (Loc.add (create_confP g0) (Loc.opp (create_confP g2))).
Proof.                                                                    
  unfold Loc.eq, LocationA.eq, MakeRing.Veq;
    intros.
  generalize confP_new_1, k_sup_1, k_inf_n; intros Hc1n Hks1 Hkin.
  unfold ImpossibilityKDividesN.Loc.eq.
  assert (Hc_idg := Hc1n idg).
  assert (Hc_g := Hc1n g0).
  assert (Hnkg0 : Z.of_nat (Pkn) <> 0).
  assert (H0n : (0 < Pkn)%nat).
  generalize k_Prime_n.
  intro.
  unfold Pkn.
  omega.
  omega.
  generalize confP_new_2; intros Hc1n2.
  unfold Loc.add, Loc.opp, ImpossibilityKDividesN.Loc.opp,
  ImpossibilityKDividesN.Loc.add, ImpossibilityKDividesN.def.n.
  set (n0 := Z.of_nat n).
  specialize (Hc1n2 (create_confP g0 - create_confP idg)).
  destruct Hc1n2.
  ...
  apply Nat.div_unique_exact in Hgcdn; try omega.
  unfold Pkn.
  rewrite <- Hgcdn.
  omega.
  generalize k_Prime_n.
  ...
  ImpossibilityKDividesN.def.n in H.
  fold n in *.
...
Qed.
*)
Lemma configP_ne_unit :
      forall g:Names.G, ~ Loc.eq (create_confP g) Loc.unit.
Proof.
  intros. 
  induction g0.
  + simpl in *.
    unfold create_confP.
    simpl in *.
    unfold Loc.mul, Loc.eq, Loc.unit, ImpossibilityKDividesN.Loc.mul, 
    ImpossibilityKDividesN.Loc.mul, LocationA.eq, Veq,
    ImpossibilityKDividesN.Loc.unit,
    ImpossibilityKDividesN.Loc.eq, ImpossibilityKDividesN.def.n.
    fold n.
    rewrite Z.mod_mod; try (generalize n_sup_1; fold n; omega).
    simpl in *.
    rewrite Z.mod_0_l, Z.mod_1_l; try (generalize n_sup_1; fold n; omega).
  + simpl in *.
    ...
    fold n.
    assert (Hgcdn := Nat.gcd_divide_l n kG).
    assert (Hmod_div := Nat.mod_divide n (Nat.gcd n kG)).
    rewrite <- Hmod_div in Hgcdn.
    assert (Hmod : Z.of_nat (n mod Nat.gcd n kG) = 0).
    omega.
    rewrite Zdiv.mod_Zmod in Hmod; try omega.
    rewrite Zdiv.Zmod_divides in *; try omega.
    rewrite Z.mod_mod.
    destruct Hmod as (x, Hmod).
    rewrite Nat2Z.inj_mul, Zdiv.div_Zdiv; try omega.
    rewrite Hmod in *.
    replace (Z.of_nat (Nat.gcd n kG) * x ) with (x * Z.of_nat (Nat.gcd n kG))
      by intuition.
    rewrite Zdiv.Z_div_mult_full.
    replace (x * Z.of_nat (Nat.gcd n kG)) with (Z.of_nat (Nat.gcd n kG) * x)
      by intuition.
    rewrite Z.mul_1_r.
    rewrite Zdiv.Zmult_mod_distr_r.
    assert (Hpkn := k_Prime_n).
    assert (1 < x).
    { assert (Hnd := no_divide).
      apply (Zmult_lt_reg_r) with (p := Z.of_nat (Nat.gcd n kG)).      
      omega.
      rewrite Z.mul_comm in Hmod.
      rewrite <- Hmod.
      rewrite Hmod_div in *; try omega.
      apply Nat.divide_pos_le in Hgcdn.
      apply le_lt_or_eq in Hgcdn.
      destruct Hgcdn.
      omega.
      assert (Hgcdkg := Nat.gcd_divide_r n kG).
      destruct Hgcdkg as (xkg, Hgcdkg).
      rewrite Hgcdkg in H0.
      assert (xkg = O).
      destruct xkg; try omega.
      rewrite H2 in H0; try omega.
      destruct xkg.
      unfold Pkn in *.
      rewrite Hgcdkg in Hnd.
      omega.
      assert (1 < S (S xkg))%nat by omega.
      assert (1 * n < S (S xkg) * n)%nat.
      apply Nat.mul_lt_mono_pos_r; omega.
      assert (n < n)%nat by omega.
      omega.
      rewrite H3 in Hgcdkg.
      simpl in *.
      omega.
      omega.
    }
    ...
    destruct H3; omega.
    generalize k_Prime_n; unfold Pkn; omega.
    generalize k_Prime_n; unfold Pkn; omega.
    omega.
    generalize k_Prime_n; unfold Pkn; omega.
    generalize k_Prime_n; unfold Pkn; omega.
...
Hypothesis Hmove : forall g,
    r.(Equiv.DGF.pgm)
        (!! (Config.map
               (apply_sim (trans (Config.loc (configP (Good g)))))
               (configP))) Loc.origin = m.
...
CoInductive AlwaysEquiv (e : execution) : Prop :=
  CAE : equiv_conf configP (execution_head e) ->
...
Lemma configP_Spectre_Equiv : forall conf g0,
      (exists k, forall id,
            Location.eq (Config.loc (conf id)) (Loc.add k (Config.loc (configP id)))
            /\
            Location.eq (Config.target (Config.robot_info (conf id)))
                        (Loc.add k (Config.target (Config.robot_info (configP id)))))
            ->
